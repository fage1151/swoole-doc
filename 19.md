...
` *****logo.png****************** `其中首部四字节\*号代表一个网络字节序的unsigned int数据，为不可见字符，第5个\*是用一个字节存储文件名长度，紧接着是文件名，接着是原始的二进制文件数据
...
json+换行，json中包含了文件名以及base64\_encode编码（会增大1/3的体积）的文件数据

### 协议样本

{"file\_name":"logo.png","file\_data":"PD9waHAKLyo......"}\\n
